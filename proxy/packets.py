...
def login_packet(context, data):
    ...
    if config.is_segaid_banned(username):
        print("[Bans] %s di Blacklist! Disconnecting..." % username)
        context.send_crypto_packet(
            packetFactory.SystemMessagePacket("Anda di Blacklist dari PCP Network.", 0x1).build())
...
def team_room_info_packet(context, data):
    ...
    if port not in blocks.blockList:
        ...
        blocks.blockList[port] = (ip_string, "Team", port)
...
def my_room_info_packet(context, data):
    ...
    if port not in blocks.blockList:
        ...
        blocks.blockList[port] = (ip_string, "Room", port)
    if port not in blocks.listeningPorts:
        from ShipProxy import ProxyFactory
        if bindIp == "0.0.0.0":
            interface_ip = myIp
...
def block_reply_packet(context, data):
    ...
    if port not in blocks.blockList:
<<<<<<< HEAD
        blocks.blockList[port] = (ip_string, "Quest", port) #not really sure if it's proper way
=======
